...
We started out by developing a small dummy model using a standard LSTM, since PyTorch does not include an mLSTM out of the box (neither does TensorFlow, for that matter). We quickly reached a model capable of unsupervised learning on predicting the next amino acid in a batch of proteins, though it was too small to actually learn anything. We then tried to implement a custom RNN in PyTorch, the mLSTM. Fortunately, creating custom RNNs in PyTorch is not that difficult and can even be optimized by PyTorch's just-in-time (JIT) compilation engine \footnote{See \url{https://pytorch.org/blog/optimizing-cuda-rnn-with-torchscript/} for details.}.

Our model architecture can be seen in figure \ref{fig:pretrain_arch}. This figure also includes the unsupervised learning setup (``pretraining'', as it is often called). The model consists of an amino acid embedding layer, which converts each index given to an amino acid to a 10-dimensional vector (this is the same as what the UniRep authors did). The resulting sequence of vectors is sent through a single-layered RNN of some form (see section \ref{section:hyperparam}). The output of the RNN are then sent through a linear layer, giving a score to each of the amino acids, which is used to predict the next amino acid in the sequence.
...
We loaded the pretrained models into the TAPE tasks, and trained on each task, until no improvement in the validation loss occurred, with a patience of 10 epochs. The training on the TAPE tasks was done on a single Nvidia Titan X GPU for all the models. Then each model was evaluated on the test split of the TAPE dataset for that task (or the ``ts115'' split for secondary structure).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/fig2b_LSTM.pdf}
    \caption{Caption}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/fig2b_GRU.pdf}
    \caption{Caption}
    \label{fig:fig2b_GRU}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/fig2c_GRU.pdf}
    \caption{Caption}
    \label{fig:fig2c_GRU}
\end{figure}
...
In addition to the above figures, we also produced numerical results, by running our models on the TAPE tasks, described earlier in \ref{section:tape}. The results can be seen in table \ref{table:tape_results}

\begin{table}[ht]
\ra{1.5}
\begin{tabularx}{\textwidth}{llllcccc}
\toprule
\textbf{Name} & \textbf{RNN} & \textbf{Size} & \textbf{TBP?} & \textbf{Sec. Str.} & \textbf{Remote Homo.} & \textbf{Fluor.} & \textbf{Stab.} \\ \hline
...
\bottomrule
\end{tabularx}
\label{table:tape_results}
\caption{Results}
\end{table}

Pretrained LSTM Secondary Structure Accuracy: 0.4269795313762456

Pretrained GRU Secondary Structure Accuracy: 0.6762725558847293

Non-pretrained LSTM Secondary Structure Accuracy: 0.6862712092647455
